"""Round 2: Fine-tune DeBERTa-v3-base on v3 data with focal loss.

Changes from Round 1 (v2):
- Data: splits_v3/ with MOF synthetic + targeted gap-fill samples added
- Loss: Class-weighted focal loss (gamma=2.0) instead of plain CrossEntropyLoss
- Focal loss down-weights easy examples and penalizes overconfident FPs

Usage:
    python training/train_classifier_v3.py
    python training/train_classifier_v3.py --gamma 2.0
    python training/train_classifier_v3.py --no_wandb
"""
...
CHECKPOINT_DIR = Path(__file__).parent / "checkpoints" / "deberta-v3-base-injection-v3"
...
class FocalLossTrainer(Trainer):
    """Trainer with class-weighted focal loss.

    Focal loss: FL(p_t) = -alpha_t * (1 - p_t)^gamma * log(p_t)

    - gamma > 0 down-weights easy/well-classified examples
    - alpha (class weights) handles class imbalance
    - Combined: focuses training on hard benign examples with trigger words
    """

    def __init__(self, class_weights: torch.Tensor, gamma: float = 2.0, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Standard CE with class weights (per-sample, no reduction)
...
def main():
    ...
    print("=" * 60)
    print("Round 2: Fine-tune DeBERTa-v3-base (v3 data + focal loss)")
    print("=" * 60)
    ...
    if not args.no_wandb:
        ...
        wandb.init(
            ...
            name=f"deberta-v3-base-v3-focal-g{args.gamma}-lr{args.lr}",
            config={
                "model": MODEL_NAME,
    ...
    # -- Save results --
    results_path = CHECKPOINT_DIR / "training_results_v3.json"
    results = {
        "model": MODEL_NAME,
        "version": "v0.3.1-round2",
        "loss": "focal",
        ...
        "data": {
            "train_samples": len(train_tokenized),
            "val_samples": len(val_tokenized),
            "hard_negatives_added": "5500+1318",
    ...
    print(f"\nNext: Run benchmark to check if focal loss + MOF data reduced NotInject FPR")
